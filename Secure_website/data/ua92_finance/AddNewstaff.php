<?php
    ...
    $staff_PhoneNumber = ""; // staff's phone number
    $staff_DateOfBirth = ""; // staff's date of birth
    $staff_Gender = ""; // staff's gender
    $staff_AnnualSalary = ""; // staffs salary
    $staff_BackgroundCheck = ""; // staffs background check
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (empty($staff_Name) || empty($staff_Address) || empty($staff_Email) || empty($staff_PhoneNumber) || empty($staff_DateOfBirth) || empty($staff_Gender) || empty($staff_AnnualSalary) || empty($staff_BackgroundCheck) ) {
            ...
            if (!$result) {
                ...
                // SQL query to insert new staff data into the 'staff' table
                $AddNewstaffSQL = "INSERT INTO staff (staff_Name, staff_Address, staff_Email, staff_PhoneNumber, staff_DateOfBirth, staff_Gender, staff_AnnualSalary, staff_BackgroundCheck)" .
                " VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
                // Prepare the SQL statement for adding a new staff record to the database
                $stmt = $connection->prepare($AddNewPstaffSQL);
                // Bind the parameters to the prepared statement for adding a new staff record
                ...
                // Execute the prepared statement to add a new staff record to the database
                $stmt->execute();
                if ($stmt->error) {
...
<body>
    ...
    <div id="Class" class="section">
        ...
        <div class="container">
            ...
            ?>
            <!-- Add New staff -->
            <form class="Class-form row " method="POST" action="AddNewstaff.php" id="ClassForm">
