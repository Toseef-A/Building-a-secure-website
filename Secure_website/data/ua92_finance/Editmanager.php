<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "GET") {
        ...
        if ($result->num_rows == 0) {
            echo "No records found for managerID: " . $managerID;
            exit;
        }
        ...
        if ($row !== null) {
            ...
            $manager_PhoneNumber = $row["manager_PhoneNumber"];
            $manager_DateOfBirth = $row["manager_DateOfBirth"];
            $manager_Gender = $row["manager_Gender"];
            $manager_AnnualSalary = $row["manager_AnnualSalary"];
            $manager_BackgroundCheck = $row["manager_BackgroundCheck"];
        }
        ...
        if (empty($managerID) || empty($manager_Name) || empty($manager_Address) || empty($manager_PhoneNumber) || empty($manager_DateOfBirth) || empty($manager_Gender) || empty($manager_Email) || empty($manager_AnnualSalary) || empty($manager_BackgroundCheck)) {
            ...
            if (!is_numeric($managerID)) {
                ...
                if (empty($errorMessage)) {
                    // Check if a manager is already assigned to the selected 
                    ...
                    if (!$result) {
                        $errorMessage = "Error fetching manager count: " . $stmt->error;
                    } else {
                        $row = $result->fetch_assoc();
                        $manager_Count = $row["manager_Count"];
    
                        if ($manager_Count > 0) {
                            // Check if the manager to be updated belongs to the selected managerID
                            $CheckmanagerSQL = "SELECT managerID FROM manager WHERE managerID = ?";
                            $stmt = $connection->prepare($CheckmanagerSQL);
                            $stmt->bind_param("i", $managerID);
                            $stmt->execute();
                            $result = $stmt->get_result();
    
                            if (!$result) {
                                $errorMessage = "Error checking manager: " . $stmt->error;
                            } else {
                                if ($result->num_rows === 0) {
                                    $errorMessage = "The manager does not belong to the selected .";
                                } else {
        ...
        <div class="container">
            ...
            <div class="section-header text-center">
                ...
                ?>
                    ...
                    <form class="Class-form row " method="POST" action="EditManager.php" id="ClassForm">
