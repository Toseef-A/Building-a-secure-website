<?php
    ...
    // Initialize managerID variable
    $managerID = "";

    // Check if the request method is GET
    if ($_SERVER["REQUEST_METHOD"] == "GET") {
        // Check if managerID is not set in GET parameters
        if (!isset($_GET["managerID"])) {
            header("location: /ua92_finance/Manager.php"); // Redirect to manager.php if managerID is not provided in GET parameters
            exit; // Exit the script to stop further execution
        }

        $managerID = $_GET["managerID"]; // Assign managerID from GET parameters to $managerID variable

        // SQL query to fetch manager data based on managerID
        $sql = "SELECT * FROM manager WHERE managerID=?";
        $stmt = $connection->prepare($sql); // Prepare the SQL statement using the connection object
        $stmt->bind_param("i", $managerID); // Bind managerID parameter to the prepared statement
        ...
        // Check if any records were found for the provided managerID
        if ($result->num_rows == 0) {
            echo "No records found for managerID: " . $managerID; // Output message if no records were found for the managerID
            exit; // Exit the script
        }

        // Fetch the first (and should be the only) row from the result set
        $row = $result->fetch_assoc();

        // Populate variables with retrieved data from the database
        if ($row !== null) {
            $manager_Name = $row["manager_Name"]; // Retrieve manager's name from the fetched row
            $manager_Address = $row["manager_Address"]; // Retrieve manager's address from the fetched row
            $manager_Email = $row["manager_Email"]; // Retrieve manager's email from the fetched row
            $manager_PhoneNumber = $row["manager_PhoneNumber"]; // Retrieve manager's phone number from the fetched row
            $manager_DateOfBirth = $row["manager_DateOfBirth"]; // Retrieve manager's date of birth from the fetched row
            $manager_Gender = $row["manager_Gender"]; // Retrieve manager's gender from the fetched row
            $manager_AnnualSalary = $row["manager_AnnualSalary"]; // Retrieve manager's annual salary from the fetched row
            $manager_BackgroundCheck = $row["manager_BackgroundCheck"]; // Retrieve manager's background check status from the fetched row
        }

        // Check if the request method is POST
    } elseif ($_SERVER["REQUEST_METHOD"] == "POST") {
        // Retrieve and sanitize form data
        $managerID = $_POST["managerID"]; // Assign managerID from POST data to $managerID variable
        ...
        if (empty($manager_Name) || empty($manager_Address) || empty($manager_Email) || empty($manager_PhoneNumber) || empty($manager_DateOfBirth) || empty($manager_Gender) || empty($manager_AnnualSalary) || empty($manager_BackgroundCheck)) {
        ...
        } elseif (!is_numeric($managerID)) {
            // Check for numeric values
            $errorMessage = "Invalid managerID";
            ...
            $UpdatemanagerSQL = "UPDATE manager SET manager_Name=?, manager_Address=?, manager_Email=?, manager_PhoneNumber=?, manager_DateOfBirth=?, manager_Gender=?, manager_AnnualSalary=?, manager_BackgroundCheck=? WHERE managerID=?";
            // Prepare the SQL statement using the connection object and the SQL query stored in $UpdateManagerSQL
            $stmt = $connection->prepare($UpdatemanagerSQL);
            $stmt->bind_param("ssssssssi", $manager_Name, $manager_Address, $manager_Email, $manager_PhoneNumber, $manager_DateOfBirth, $manager_Gender, $manager_AnnualSalary, $manager_BackgroundCheck, $managerID);
            ...
            // The first argument "ssssssss" specifies the types of the variables being bound, where "s" stands for string and "i" stands for interger
            ...
            // are all expected to be strings and $managerID is expected to be an integer. The following variables are the actual values to be bound to the placeholders in the SQL statement.
            ...
            if ($stmt->error) {
                ...
                // Set success message
                $successMessage = "Manager has been updated";
            }
        }
    }
...
<body>
    ...
    <div class="container">
        ...
        <div class="section-header text-center mb-4">
            <h2 class="title">Edit Manager</h2>
        </div>

        <!-- Form section -->
        <form method="POST" action="EditManager.php"> <!-- Form starts, uses POST method, submits to EditManager.php -->

            <!-- Hidden input for managerID -->
            <input type="hidden" name="managerID" value="<?php echo htmlspecialchars($managerID); ?>">
