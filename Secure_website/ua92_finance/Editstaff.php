<?php
    ...
    // Initialize staffID variable
    $staffID = "";

    // Check if the request method is GET
    if ($_SERVER["REQUEST_METHOD"] == "GET") {
        // Check if staffID is not set in GET parameters
        if (!isset($_GET["staffID"])) {
            header("location: /ua92_finance/staff.php"); // Redirect to staff.php if staffID is not provided in GET parameters
            exit; // Exit the script to stop further execution
        }

        $staffID = $_GET["staffID"]; // Assign staffID from GET parameters to $staffID variable

        // SQL query to fetch staff data based on staffID
        $sql = "SELECT * FROM staff WHERE staffID=?";
        $stmt = $connection->prepare($sql); // Prepare the SQL statement using the connection object
        $stmt->bind_param("i", $staffID); // Bind staffID parameter to the prepared statement
        ...
        // Check if any records were found for the provided staffID
        if ($result->num_rows == 0) {
            echo "No records found for staffID: " . $staffID; // Output message if no records were found for the staffID
            exit; // Exit the script
        }
        ...
        if ($row !== null) {
            $staff_Name = $row["staff_Name"]; // Retrieve staff's name from the fetched row
            $staff_Address = $row["staff_Address"]; // Retrieve staff's address from the fetched row
            $staff_Email = $row["staff_Email"]; // Retrieve staff's email from the fetched row
            $staff_PhoneNumber = $row["staff_PhoneNumber"]; // Retrieve staff's phone number from the fetched row
            $staff_DateOfBirth = $row["staff_DateOfBirth"]; // Retrieve staff's date of birth from the fetched row
            $staff_Gender = $row["staff_Gender"]; // Retrieve staff's gender from the fetched row
            $staff_AnnualSalary = $row["staff_AnnualSalary"]; // Retrieve staff's annual salary from the fetched row
            $staff_BackgroundCheck = $row["staff_BackgroundCheck"]; // Retrieve staff's background check status from the fetched row
        }

        // Check if the request method is POST
    } elseif ($_SERVER["REQUEST_METHOD"] == "POST") {
        // Retrieve and sanitize form data
        $staffID = $_POST["staffID"]; // Assign staffID from POST data to $staffID variable
        ...
        if (empty($staff_Name) || empty($staff_Address) || empty($staff_Email) || empty($staff_PhoneNumber) || empty($staff_DateOfBirth) || empty($staff_Gender) || empty($staff_AnnualSalary) || empty($staff_BackgroundCheck)) {
            ...
            // Prepare the SQL statement using the connection object and the SQL query stored in $UpdatestaffSQL
            ...
            // are all expected to be strings and $staffID is expected to be an integer. The following variables are the actual values to be bound to the placeholders in the SQL statement.
...
<body>
    ...
    <div class="container">
        ...
        <!-- Form section -->
        <form method="POST" action="Editstaff.php"> <!-- Form starts, uses POST method, submits to Editstaff.php -->

            <!-- Hidden input for staffID -->
